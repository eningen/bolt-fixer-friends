      ...
      <main className="mx-auto flex h-[calc(100vh-4rem)] max-w-2xl flex-col px-4 py-4 pb-24 md:pb-4">
        ...
        <div className="flex-1 space-y-3 overflow-y-auto py-4">
          {messagesPending ? <div className="text-center text-sm text-muted-foreground">メッセージを読み込み中…</div> : messages.length === 0 ? <div className="py-20 text-center text-sm text-muted-foreground">まだメッセージはありません。最初の一言を送ってみよう！</div> : messages.map((message) => {
            ...
            return <div key={message.id} className={`flex ${mine ? "justify-end" : "justify-start"}`}><div className={`max-w-[82%] rounded-2xl px-4 py-2.5 text-sm ${mine ? "rounded-br-md bg-primary text-primary-foreground" : "rounded-bl-md bg-muted text-foreground"}`}><p className="whitespace-pre-wrap break-words">{message.body}</p><div className={`mt-1 flex items-center gap-1 text-[10px] ${mine ? "justify-end text-primary-foreground/70" : "text-muted-foreground"}`}><span>{new Date(message.created_at).toLocaleString("ja-JP", { month: "numeric", day: "numeric", hour: "2-digit", minute: "2-digit" })}</span>{mine && <span aria-label={message.read ? "既読" : "未読"}>・{message.read ? "既読" : "未読"}</span>}</div></div></div>;
          })}
          <div ref={bottomRef} />
        </div>
