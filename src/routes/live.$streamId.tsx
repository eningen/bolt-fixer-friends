...
function LiveDetailPage() {
  ...
  return <div className="min-h-screen"><Header /><main className="mx-auto max-w-5xl px-4 py-8 pb-24">
    ...
    <div className="mt-4 grid gap-6 lg:grid-cols-[1fr_360px]">
      <section>
        <div className="overflow-hidden rounded-2xl bg-black shadow-sm"><video ref={videoRef} autoPlay playsInline muted={isHost} controls={!isHost} className="aspect-video w-full object-contain" />{isHost && !mediaStream ? <div className="flex aspect-video items-center justify-center text-sm text-white"><Loader2 className="mr-2 size-4 animate-spin" />カメラを準備中…</div> : null}{!isHost && stream.status === "live" && viewer.state !== "connected" ? <div className="flex aspect-video items-center justify-center text-sm text-white"><Loader2 className="mr-2 size-4 animate-spin" />配信に接続中…</div> : null}</div>
        ...
        <div className="mt-4 flex items-center gap-3"><UserAvatar profile={stream.profile ?? undefined} /><div><p className="font-semibold">{stream.profile?.display_name ?? stream.profile?.username ?? "配信者"}</p><p className="text-xs text-muted-foreground">@{stream.profile?.username ?? ""}</p></div></div>
      </section>
      <section className="flex min-h-[520px] flex-col rounded-2xl border border-border bg-surface/30 p-4"><h2 className="mb-3 font-bold">💬 ライブチャット</h2><div className="min-h-0 flex-1 space-y-3 overflow-y-auto">{chat.length === 0 ? <p className="py-10 text-center text-sm text-muted-foreground">最初のコメントを送ってみよう！</p> : chat.map((item) => <div key={item.id} className="flex gap-2"><UserAvatar profile={item.profile ?? undefined} className="size-8" /><div className="min-w-0"><p className="text-xs font-semibold">{item.profile?.display_name ?? item.profile?.username ?? "ユーザー"}</p><p className="break-words text-sm">{item.body}</p></div></div>)}</div><div className="mt-3 flex gap-2"><Input value={message} onChange={(e) => setMessage(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") void sendChat(); }} maxLength={500} placeholder="コメントを入力…" disabled={stream.status !== "live"} /><Button size="icon" onClick={() => void sendChat()} disabled={!message.trim() || stream.status !== "live"} aria-label="送信"><Send className="size-4" /></Button></div></section>
    </div>
