...
export function SideNav() {
  ...
  const items: Item[] = [
    { label: "ホーム", icon: Home, to: "/", match: (p) => p === "/" },
    { label: "検索", icon: Search, to: "/search", match: (p) => p.startsWith("/search") },
    { label: "ランキング", icon: Trophy, to: "/ranking", match: (p) => p.startsWith("/ranking") },
    { label: "投稿", icon: Upload, to: "/upload", match: (p) => p.startsWith("/upload") },
    profile
      ? {
          label: "マイページ",
          icon: UserRound,
