  ...
  return <header className="sticky top-0 z-50 border-b border-border bg-background/95 backdrop-blur">
    <div className="mx-auto flex h-14 max-w-6xl items-center gap-3 px-4">
      ...
      <nav className="flex shrink-0 items-center gap-1">
        ...
        {user ? <><Button asChild size="sm"><Link to="/upload"><Upload className="size-4" /><span className="hidden sm:inline">{t("upload")}</span></Link></Button><Button asChild variant="ghost" size="sm" className="relative" aria-label={t("notifications")}><Link to="/notifications"><Bell className="size-4" />{unreadCount > 0 ? <span className="absolute -right-0.5 -top-0.5 flex min-w-4 items-center justify-center rounded-full bg-destructive px-1 text-[9px] font-bold text-destructive-foreground">{unreadCount > 99 ? "99+" : unreadCount}</span> : null}</Link></Button>{profile ? <Link to="/u/$username" params={{ username: profile.username }} aria-label={t("myPage")}><Avatar className="size-8"><AvatarImage src={profile.avatar_url ?? undefined} alt="" /><AvatarFallback className="text-xs">{profile.display_name.slice(0, 2)}</AvatarFallback></Avatar></Link> : null}<Button variant="ghost" size="sm" onClick={onSignOut} aria-label={t("logout")}><LogOut className="size-4" /></Button></> : <Button asChild size="sm"><Link to="/auth">{t("login")}</Link></Button>}
        <Dialog open={settingsOpen} onOpenChange={setSettingsOpen}>
          ...
          <DialogContent className="sm:max-w-md"><DialogHeader><DialogTitle>{t("settings")}</DialogTitle></DialogHeader><div className="grid gap-3 py-2"><div className="rounded-lg border border-border p-3"><div className="mb-2 flex items-center gap-2 font-medium">🌐 {t("languageSettings")}</div><select id="language-select" value={language} onChange={(event) => setLanguage(event.target.value as "ja" | "en")} className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"><option value="ja">{t("japanese")}</option><option value="en">{t("english")}</option></select></div><Button variant="outline" className="justify-start" onClick={() => { setSettingsOpen(false); void navigate({ to: "/terms" }); }}><FileText className="mr-2 size-4" />{t("terms")}</Button><Button variant="outline" className="justify-start" onClick={() => { setSettingsOpen(false); void navigate({ to: "/admin-login" }); }}><Shield className="mr-2 size-4" />{t("adminLogin")}</Button></div></DialogContent>
        </Dialog>
      </nav>
    </div>
  </header>;
