...
import { Bell, LogOut, Search, Settings, Trophy, Upload } from "lucide-react";
...
export function Header() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`mailbox-notifications-${user.id}`)
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "mailbox_messages", filter: `user_id=eq.${user.id}` }, (payload) => {
        const row = payload.new as { title?: string; body?: string };
        void queryClient.invalidateQueries({ queryKey: ["mailbox", user.id] });
        playNotificationSound();
        toast.info(row.title ?? "新しいお知らせ", { description: row.body ?? "メールボックスに新しいお知らせがあります" });
        if (typeof Notification !== "undefined" && Notification.permission === "granted") {
          try { new Notification(row.title ?? "Stickman video", { body: row.body ?? "新しいお知らせがあります" }); } catch { /* ignored */ }
        }
      })
      .subscribe();
      ...
      <div className="mx-auto flex h-14 max-w-6xl items-center gap-3 px-4">
        ...
        <nav className="flex shrink-0 items-center gap-1">
          ...
          <Dialog open={settingsOpen} onOpenChange={setSettingsOpen}>
            ...
            <DialogContent className="sm:max-w-md"><DialogHeader><DialogTitle>{t("languageSettings")}</DialogTitle></DialogHeader><div className="grid gap-2 py-2"><label htmlFor="language-select" className="text-sm font-medium">{t("language")}</label><select id="language-select" value={language} onChange={(event) => setLanguage(event.target.value as "ja" | "en")} className="h-10 rounded-md border border-input bg-background px-3 text-sm"><option value="ja">{t("japanese")}</option><option value="en">{t("english")}</option></select></div></DialogContent>
          </Dialog>
        </nav>
      </div>
    </header>
