...
CREATE TABLE IF NOT EXISTS public.notifications (
  ...
  type text NOT NULL CHECK (type IN ('like','comment','subscribe','new_video','friend_request')),
...
GRANT SELECT, UPDATE ON public.friend_requests TO authenticated;
DROP POLICY IF EXISTS friend_requests_select_participant ON public.friend_requests;
CREATE POLICY friend_requests_select_participant ON public.friend_requests FOR SELECT TO authenticated USING (auth.uid() = requester_id OR auth.uid() = recipient_id);
...
CREATE POLICY friendships_select_participant ON public.friendships FOR SELECT TO authenticated USING (auth.uid() = user_a OR auth.uid() = user_b);
...
DROP POLICY IF EXISTS direct_messages_select_friend ON public.direct_messages;
CREATE POLICY direct_messages_select_friend ON public.direct_messages FOR SELECT TO authenticated USING ((auth.uid() = sender_id OR auth.uid() = recipient_id) AND EXISTS (SELECT 1 FROM public.friendships f WHERE f.user_a = LEAST(sender_id, recipient_id) AND f.user_b = GREATEST(sender_id, recipient_id)));
DROP POLICY IF EXISTS direct_messages_insert_friend ON public.direct_messages;
CREATE POLICY direct_messages_insert_friend ON public.direct_messages FOR INSERT TO authenticated WITH CHECK (auth.uid() = sender_id AND sender_id <> recipient_id AND EXISTS (SELECT 1 FROM public.friendships f WHERE f.user_a = LEAST(sender_id, recipient_id) AND f.user_b = GREATEST(sender_id, recipient_id)));

CREATE OR REPLACE FUNCTION public.send_friend_request(p_recipient_id uuid)
RETURNS uuid LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE v_requester_id uuid := auth.uid(); v_request_id uuid; v_existing uuid;
BEGIN
  ...
  INSERT INTO public.notifications(user_id,actor_id,type,metadata) VALUES (p_recipient_id,v_requester_id,'friend_request',jsonb_build_object('request_id',v_request_id));
  RETURN v_request_id;
END;
$$;
...
SELECT EXISTS(SELECT 1 FROM public.friendships WHERE user_a=LEAST(auth.uid(),p_other_id) AND user_b=GREATEST(auth.uid(),p_other_id));
$$;
GRANT EXECUTE ON FUNCTION public.are_friends(uuid) TO authenticated;

CREATE OR REPLACE FUNCTION public.update_friend_request_timestamp() RETURNS trigger LANGUAGE plpgsql SET search_path=public AS $$ BEGIN NEW.updated_at=now(); RETURN NEW; END; $$;
